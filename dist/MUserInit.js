...
exports.MUserInit = void 0;
const fs = __importStar(require("fs-extra"));
class MUserInit {
    constructor(mapFileName, tplDirName) {
        /*
         * -----
         * json file
         * -----
         */
        if (!fs.pathExistsSync(`./${mapFileName}.json`)
...
`);
        ...
        if (!fs.pathExistsSync(`./${tplDirName}/sample/sample={name}.js.ejs`)) {
            ...
            fs.outputFileSync(`./${tplDirName}/sample/sample={name}.js.ejs`, `// <%= comment %>
<%#_ ejs comment _%>
...
`);
        ...
        if (!fs.pathExistsSync(`./${tplDirName}/component={name}.tsx.ejs`)) {
            fs.outputFileSync(`./${tplDirName}/component={name}.tsx.ejs`, `/** @jsxImportSource @emotion/react */
...
`);
        ...
        if (!fs.pathExistsSync(`./${tplDirName}/index=index.ts.ejs`)) {
            fs.outputFileSync(`./${tplDirName}/index=index.ts.ejs`, `<%_ for (var i = 0; i < brings.length; i++) { _%>
...
`);
        ...
        if (!fs.pathExistsSync(`./${tplDirName}/story={name}.stories.tsx.ejs`)) {
            fs.outputFileSync(`./${tplDirName}/story={name}.stories.tsx.ejs`, `import { Meta, Story } from '@storybook/react';
...
`);
        ...
        if (!fs.pathExistsSync(`./${tplDirName}/test={name}.test.tsx.ejs`)) {
            fs.outputFileSync(`./${tplDirName}/test={name}.test.tsx.ejs`, `import { render, screen } from '@testing-library/react';
<%_ for (var i = 0; i < brings.length; i++) { _%>
